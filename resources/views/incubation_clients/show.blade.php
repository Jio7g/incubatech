                            ...
                            <tr class="hover:bg-gray-100">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                        ...
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('actualizaciones.create', $incubation->id) }}" class="bg-green-500 hover:bg-green-600 text-white font-bold py-2 px-4 rounded flex items-center">
                                                <svg class="h-5 w-5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z" />
                                                </svg>
                                                Actualizar
                                            </a>
                                            <a href="{{ route('actualizaciones.index', ['incubacion_id' => $incubation->id]) }}" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded flex items-center">
                                                <svg class="h-5 w-5 mr-2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
...
<script>
    ...
    function searchIncubations() {
        ...
        rows.forEach(row => {
            ...
            const huevosNoFertiles = row.cells[7].textContent.toLowerCase();
            const huevosEclosionados = row.cells[8].textContent.toLowerCase();
            const huevosEnProceso = row.cells[9].textContent.toLowerCase();

            if (fechaRecepcion.includes(searchTerm) || producto.includes(searchTerm) || cantidad.includes(searchTerm) || tipoHuevo.includes(searchTerm) || numeroBandeja.includes(searchTerm) || etapa.includes(searchTerm) || estado.includes(searchTerm) || huevosNoFertiles.includes(searchTerm) || huevosEclosionados.includes(searchTerm) || huevosEnProceso.includes(searchTerm)) {
                row.style.display = '';
            } else {
